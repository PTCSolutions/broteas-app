...
<div class="p-4 h-full">
	<div class="bg-white dark:bg-gray-600 rounded-lg p-4 flex flex-row">
		...
		<div class="flex flex-col">
			...
			<div class="h-2" />
			<a href={`/artists/${album.artists[0].id}`} class="text-xl hover:underline"
	...
	<div>
		{#await getPostsForObject(album.id)}
			<div>Loading</div>
		{:then comments}
			...
			{:else}
				<div>
					Looks like no one is talking about {album.name}, be the first to start the discussion today!
				</div>
			{/if}
		{:catch error}
			<div>{error}</div>
